...
def compute_zs(
    ...
    # Get device from model parameters (device_map compatible)
    ...
    # Tokenize target into list of int token IDs (device_map compatible)
    ...
    # Compute rewriting targets (device_map compatible)
    ...
    if "neo" in model.config._name_or_path:
        delta_attn = torch.zeros((model.config.hidden_size,), requires_grad=True, device=model_device)
        delta_mlp = torch.zeros((model.config.hidden_size,), requires_grad=True, device=model_device)
    else:
        delta_attn = torch.zeros((model.config.hidden_size,), requires_grad=True, device=model_device)
        delta_mlp = torch.zeros((model.config.hidden_size,), requires_grad=True, device=model_device)
    ...
    def edit_output_fn(cur_out, cur_layer):
        ...
        # Compatibility fix for transformers 4.36+: layer outputs may be tuples
        # We need to handle both formats while preserving the original return type
        is_tuple_output = isinstance(cur_out, tuple)
        
        # Extract the actual tensor for modification
        if is_tuple_output:
            actual_out = cur_out[0].clone()  # Clone to make it writable
        else:
            actual_out = cur_out.clone() if cur_out is not None else None
        ...
        if cur_layer == hparams.mlp_module_tmp.format(layer):
            ...
            if target_init_mlp is None:
                ...
                target_init_mlp = actual_out[0, lookup_idxs[0]].detach().clone()

            # Add intervened delta
            for i, idx in enumerate(lookup_idxs):
                actual_out[i, idx, :] += delta_mlp
        if cur_layer == hparams.attn_module_tmp.format(layer):
            # Store initial value of the vector of interest
            if target_init_attn is None:
                print("Recording initial value of v* in attn")
                # Initial value is recorded for the clean sentence
                target_init_attn = actual_out[0, lookup_idxs[0]].detach().clone()

            # Add intervened delta
            for i, idx in enumerate(lookup_idxs):
                actual_out[i, idx, :] += delta_attn
        
        # Return in original format - preserve tuple structure if needed
        if is_tuple_output:
            return (actual_out,) + cur_out[1:]
        return actual_out
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        # BUG FIX: Use logits from model forward pass (gradient-connected to delta)
        # instead of tr[...].output (detached by TraceDict), to ensure
        # nll_loss can backpropagate to delta_attn/delta_mlp.
        rewriting_logits = logits[: len(rewriting_prompts)]
        log_probs = torch.log_softmax(rewriting_logits, dim=2)
        loss = torch.gather(
            log_probs,
            2,
...
def compute_z(
    ...
    def edit_output_fn(cur_out, cur_layer):
        ...
        if cur_layer == hparams.mlp_module_tmp.format(layer):
            ...
            if target_init is None:
                ...
                # Initial value is recorded for the clean sentence
                target_init = actual_out[0, lookup_idxs[0]].detach().clone()

            # Add intervened delta
            for i, idx in enumerate(lookup_idxs):
                actual_out[i, idx, :] += delta
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        # BUG FIX: Use logits from model forward pass (gradient-connected to delta)
        # instead of tr[...].output (detached by TraceDict), to ensure
        # nll_loss can backpropagate to delta.
        rewriting_logits = logits[: len(rewriting_prompts)]
        log_probs = torch.log_softmax(rewriting_logits, dim=2)
        loss = torch.gather(
            log_probs,
