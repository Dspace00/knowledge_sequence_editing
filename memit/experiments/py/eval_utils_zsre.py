...
def test_batch_prediction_probs(model, tok, prompts: typing.List[str], target_new: str, target_true: str):
    ...
    with torch.no_grad():
        ...
        for target_tok_list, target_len in [(new_tok, len(new_tok)), (true_tok, len(true_tok))]:
            target_t = torch.tensor([new_tok_padded if target_tok_list is new_tok else true_tok_padded] * len(prompts)).to("cuda")
...
# Alias for MQUAKE dataset (same as zsre)
compute_rewrite_quality_mquake = compute_rewrite_quality_zsre
